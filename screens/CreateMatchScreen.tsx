...
export default function CreateMatchScreen() {
  ...
  const createMatch = async (playNow: boolean = false) => {
    setLoading(true);
    try {
      // Remove manual matchId generation
      // const matchId = Date.now().toString();
      const newMatch = {
        // id will be auto-generated by DB (UUID)
        ...
        currentOvers: '0.0',
        createdAt: Date.now(),
        scheduled_at: new Date().toISOString(), // Set to current time
        creatorId: user?.id || '',
      };

      console.log('DEBUG: Creating match with status:', newMatch.status, newMatch);
      console.log('DEBUG: scheduled_at timestamp:', new Date(newMatch.scheduled_at).getTime());
      console.log('DEBUG: current timestamp:', Date.now());
      console.log('DEBUG: scheduled_at ISO string:', newMatch.scheduled_at);
      console.log('DEBUG: scheduled_at date object:', new Date(newMatch.scheduled_at));
      // Insert match and get real match id
      const { data: inserted, error: matchError } = await db?.from('matches').insert([newMatch]).select();
      console.log('Match insert result:', { inserted, matchError });
      if (inserted && inserted[0]) {
        console.log('DEBUG: Inserted match scheduled_at:', inserted[0].scheduled_at);
        console.log('DEBUG: Inserted match created_at:', inserted[0].created_at);
      }
      if (matchError || !inserted || !inserted[0]) throw matchError || new Error('Match insert failed');
      const realMatchId = inserted[0].id;
      const teamAId = inserted[0].team1_id;
      const teamBId = inserted[0].team2_id;

      // Insert zeroed match_performances for all players using realMatchId
      ...
      console.log('About to insert match_performances:', performanceRows);
      const { data: perfData, error: perfError } = await db?.from('match_performances').insert(performanceRows);
      if (perfError) {
        console.error('Error inserting match_performances:', perfError);
      } else {
        console.log('Inserted match_performances:', perfData);
      }
      ...
      // Always navigate to LiveMatchSummaryScreen after match creation
      navigation.navigate('LiveMatchSummaryScreen', { matchId: realMatchId });
    } catch (error) {
